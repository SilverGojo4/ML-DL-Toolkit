...
# 函數: 使用隨機搜索的 Random Forest 分類模型
def rf_classify_random_search(feature_train, label_train, param_distributions, 
    ...
    - model_random_state: 初始化 RandomForestClassifier 的隨機種子 (int, 預設為 42)
    - search_random_state: 初始化 RandomizedSearchCV 的隨機種子 (int, 預設為 42)
    - class_weight: 類別權重參數 (dict, 'balanced', 'balanced_subsample', 或 None, 預設為 None)
    ...
    返回: 
    - 通過 RandomizedSearchCV 找到的最佳 Random Forest 分類器 (RandomForestClassifier 物件)
    ...
    try:
        ...
        # 對訓練數據進行擬合
        rf_random_search.fit(feature_train, label_train)

        # 獲得最佳估計器 (最佳 Random Forest 模型)
        best_rf_model = rf_random_search.best_estimator_

        # 打印最佳 Random Forest 分類模型的詳細資訊
        ...
        for param, value in rf_random_search.best_params_.items():
            print(f"{param}: {value}")
        print("-" * 80)
        print(f"Validation Set Score: {rf_random_search.best_score_}")
        print("=" * 80 + "\n")
        ...
        # 錯誤處理
        print(f"An error occurred during the Random Forest Classification training process:\n{e}")
        return None

# 函數: 使用隨機搜索的 Random Forest 回歸模型
def rf_regress_random_search(feature_train, target_train, param_distributions, 
                             model_random_state=42, search_random_state=42, 
    ...
    返回: 
    - 通過 RandomizedSearchCV 找到的最佳 Random Forest 回歸器 (RandomForestRegressor 物件)
      如果訓練過程中發生錯誤，則返回 None
    """
    ...
    try:
        ...
        rf = RandomForestRegressor(random_state=model_random_state)
        ...
        # 對訓練數據進行擬合
        rf_random_search.fit(feature_train, target_train)

        # 獲得最佳估計器 (最佳 Random Forest 回歸模型)
        best_rf_model = rf_random_search.best_estimator_

        # 打印最佳 Random Forest 回歸模型的詳細資訊
        ...
        # 錯誤處理
        print(f"An error occurred during the Random Forest Regression training process:\n{e}")
        return None

# 函數: 輸出特徵重要性至 CSV 檔案
def rf_importance_score_to_csv(model, feature_names, output_file):
    """
    參數: 
    - model: 訓練完成的 Random Forest 模型 (RandomForestClassifier 或 RandomForestRegressor 物件)
